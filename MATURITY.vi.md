...
2. **Cơ Chế Ký Quỹ Escrow Tự Động (Đợt 2 Chưa Hoàn Tất)**:
   - Hiện tại, luồng thanh toán trong bản Live là thanh toán P2P trực tiếp (Direct Settlement) tách 2 hóa đơn Lightning song song: thanh toán tiền phòng trực tiếp cho Host theo tỷ lệ Profit Sharing và tự động trích % phí protocol vào Quỹ Thưởng Hạ Tầng (Treasury RFC-0016).
...
3. **Cơ Chế Điều Chỉnh Phí Protocol Do Quản Trị Viên (Admin) Đặt — Chưa Phải Đồng Thuận Phi Tập Trung (DAO Consensus)**:
   - Tỷ lệ phí Protocol (`baseFeeRatePcm`) hiện được cập nhật trực tiếp bởi khóa quản trị viên ủy quyền (Admin Pubkey Whitelist) thông qua xác thực NIP-98 (Kind 27235).
   - Cơ chế biểu quyết phân tán cộng đồng (On-chain DAO / Nostr Vote Consensus) cho tham số phí chưa được kích hoạt do bài toán phòng chống tấn công Sybil trên phiếu bầu chưa giải quyết triệt để.
   - Để đảm bảo tính minh bạch cypherpunk, mọi thay đổi phí đều bắt buộc ký bằng khóa Schnorr của Admin và phát tán công khai thành sự kiện Nostr trên các relay (`t: cypherguide-fee`) làm Audit Trail bất biến cho toàn mạng lưới giám sát.
...
16. **Tự Động Trích Phí Booking Vào Treasury (RFC-0016 Lightning Payout Flow)**: Tách luồng thanh toán thành 2 invoice Lightning độc lập: Invoice 1 thanh toán tiền phòng Host (`totalPriceSats`), Invoice 2 trích phí protocol (`protocolFeeSats`) chuyển thẳng vào ví Quỹ Thưởng Hạ Tầng (`infraIncentiveTreasuryLightningAddress`). Xác nhận thanh toán qua Preimage SHA-256 bắt buộc cả 2 giao dịch thành công mới chuyển trạng thái booking sang 'paid'. Tự động gộp phí vào invoice Host nếu phí quá nhỏ dưới ngưỡng minSendable.
17. **Cấu Hình Phí Protocol Nguồn Sự Thật Duy Nhất & Audit Trail Nostr Bất Biến (RFC-0016)**: Loại bỏ giá trị hardcode `baseFeeRatePcm` trong `dynamicFee.ts` và số `0.05` tĩnh trong store. Nối một nguồn sự thật duy nhất từ `data/protocol_config.json` qua `GET /api/protocol/config`. Bảo vệ endpoint `PATCH /api/protocol/fee` bằng xác thực NIP-98 (Kind 27235). Tự động ký và phát tán sự kiện Nostr công khai lên các relay giao thức mỗi khi thay đổi phí, công khai minh bạch lịch sử đổi phí trên `GovernancePanel` và tính toán chuẩn xác trong `BookingModal`.
