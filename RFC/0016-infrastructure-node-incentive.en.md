...
- **Economic centralization risk:** this is a recurring, empirically observed pattern in decentralized incentive networks — not a hypothetical. Bitcoin mining consolidated into a handful of large pools due to economies of scale (cheap electricity, ASIC farms); Helium's Proof-of-Coverage was gamed by "hotspot farmers" clustering large numbers of devices to maximize rewards, distorting the actual coverage picture; Filecoin storage power concentrated among a few well-capitalized operators. The same risk applies to RFC-0016: a sufficiently capitalized actor could stand up many relays and have them witness each other, and even with a minimum reputation requirement (RFC-0003) could build enough valid identities over time to clear that bar — turning a "decentralized" network into a handful of large players quietly capturing most of the reward pool.
  - **Proposed mitigation direction (open for discussion, not finalized):** reward with declining marginal returns per additional node controlled by the same operator/identity cluster, rather than linear reward scaling with node count. This requires a cluster-detection mechanism (e.g. anomalous IP-geography/latency patterns, identity age and activity heuristics) — which is itself another oracle problem, and would warrant its own RFC if pursued.
