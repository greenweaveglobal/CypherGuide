...
- **Rủi ro tập trung hóa kinh tế (Economic Centralization Risk):** đây là hiện tượng đã xảy ra lặp lại ở nhiều mạng lưới incentive phi tập trung — không phải giả thuyết. Bitcoin mining dồn về vài mining pool lớn nhờ lợi thế quy mô (điện rẻ, ASIC farm); Helium Proof-of-Coverage bị "hotspot farmer" dựng hàng loạt thiết bị tập trung để tối đa reward, làm sai lệch bức tranh coverage thật; Filecoin storage power tập trung ở vài công ty vốn lớn. Rủi ro tương tự áp lên RFC-0016: một actor đủ vốn có thể dựng nhiều relay + witness cho nhau, và dù có yêu cầu reputation tối thiểu (RFC-0003) thì họ vẫn có thể build đủ nhiều identity hợp lệ theo thời gian để vượt qua rào cản đó — biến network "phi tập trung" thành vài big player thao túng ngầm phần lớn reward pool.
  - **Hướng giảm thiểu đề xuất (cần thảo luận thêm, chưa chốt):** giới hạn reward theo hàm nghịch biến với số node cùng 1 operator/identity cluster sở hữu (declining marginal reward per additional node), thay vì reward tuyến tính theo số node vận hành. Cần cơ chế phát hiện cluster (ví dụ theo địa lý IP/latency pattern bất thường, theo tuổi đời + hoạt động của identity) — bản thân cơ chế phát hiện này cũng là 1 oracle problem khác, cần RFC riêng nếu triển khai.
