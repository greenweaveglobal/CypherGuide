...
8. **Compute-as-a-Stay (RFC-0013) (Draft/Experimental Tier)**: Extends the concept of 'guest' to autonomous AI agents renting idle compute resources (CPU, RAM, bandwidth) on host SBC/LoRa nodes. Invariant constraints: Financial authorization and payment must strictly remain with the human npub owning the agent with an upfront hard spending cap; verification via objective machine metrics; segregated reputation namespace; fixed pricing with standard escrow; host legal liabilities preserved under RFC-0012.
