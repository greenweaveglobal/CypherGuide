...
2. **Automated Escrow Custody (Phase 2 Under Active Development)**:
   - Currently, Live mode transactions operate as dual parallel Lightning invoices: direct peer-to-peer (P2P) room settlement to the Host's and co-owners' Lightning Addresses (LUD-16) per agreed Profit Sharing ratios, alongside automatic protocol fee extraction to the Infrastructure Incentive Treasury (Treasury RFC-0016).
...
3. **Protocol Fee Parameterization Set by Authorized Admin — Not Distributed DAO Consensus**:
   - The protocol fee rate (`baseFeeRatePcm`) is currently updated directly by authorized protocol admin keys (Admin Pubkey Whitelist) via NIP-98 HTTP Authentication (Kind 27235).
   - Distributed community voting (On-chain DAO / Nostr Vote Consensus) for fee parameters is not enabled yet until Sybil-resistant voting mechanisms are mathematically resolved.
   - To preserve cypherpunk transparency, every fee adjustment is cryptographically signed with the Admin's Schnorr key and publicly published as an immutable Nostr audit event across protocol relays (`t: cypherguide-fee`).
...
16. **Automated Protocol Fee Extraction to Treasury (RFC-0016 Lightning Payout Flow)**: Splits payment flow into two independent parallel Lightning invoices: Invoice 1 settles host room price (`totalPriceSats`), Invoice 2 extracts dynamic protocol fees (`protocolFeeSats`) directly into the Infrastructure Incentive Treasury (`infraIncentiveTreasuryLightningAddress`). Enforces cryptographic SHA-256 preimage verification on both invoices before marking booking as 'paid'. Auto-merges fee into host invoice if protocol fee is below sendable threshold.
17. **Protocol Fee Single Source of Truth & Transparent Nostr Audit Trail (RFC-0016)**: Eliminated hardcoded `baseFeeRatePcm` in `dynamicFee.ts` and legacy static `0.05` state in store. Unified under a single authoritative source loaded from `data/protocol_config.json` via `GET /api/protocol/config`. Secured `PATCH /api/protocol/fee` with NIP-98 (Kind 27235). Automatically signs and broadcasts public Nostr events across protocol relays for every fee adjustment, displaying transparent history on `GovernancePanel` and accurate calculations in `BookingModal`.
